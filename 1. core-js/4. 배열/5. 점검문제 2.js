...
while (arr.length > 0) {
    let userdel = prompt(`현재멤버: [${arr}]\n삭제할 멤버 이름을 입력해주세요.`);

    if (arr.indexOf(userdel) === -1) {
        alert(`${userdel}(은)는 멤버 목록에 없는 사람입니다. 재입력하세요.`);
        continue;
    }
    else {
        let deldata = arr.indexOf(userdel);
        arr.splice(deldata,1);
    }
}

alert(`모든 멤버를 살해하셨습니다.\n수고하셨습니다.`);